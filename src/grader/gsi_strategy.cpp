...
int gsi_sequence(const std::vector<game::Action>& round_actions) {
    int raises_used =
        static_cast<int>(std::count(round_actions.begin(), round_actions.end(), game::kRihActionRaise));
    bool opened_with_check = !round_actions.empty() && round_actions.front() == game::kRihActionCallCheck;
    return opened_with_check ? raises_used : raises_used + 3;
}

int decision_type(const std::vector<game::Action>& round_actions) {
    if (round_actions.empty() || round_actions.back() != game::kRihActionRaise) return 0;
    return static_cast<int>(std::count(round_actions.begin(), round_actions.end(), game::kRihActionRaise));
}
...
GsiStrategy::PublicContext GsiStrategy::describe(const game::State& state) {
    game::RihParsedHistory parsed = game::rih_parse_history(state);
    const std::vector<game::Action>& round_actions = game::rih_active_round_actions(parsed);
    ...
    if (parsed.board_cards_dealt >= 1) {
        ...
        context.sequence1 = gsi_sequence(parsed.round1_actions);
    }
    if (parsed.board_cards_dealt >= 2) {
        context.board1 = gsi_card(static_cast<Card>(state.public_cards[1]));
        context.sequence2 = gsi_sequence(parsed.round2_actions);
    }
    return context;
}
